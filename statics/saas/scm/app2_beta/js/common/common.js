	...
	$('#quickAddVendor').on('click', function(e){
		...
		$.dialog({
			...
			data: {oper: 'add', callback: function(data, oper, dialogWin){
				...
				settlementAccountCombo.loadData('../basedata/settAcct?action=query', ['id', data.id]);
				dialogWin && dialogWin.api.close();
			}},
			width : 640,
			height : 205,
	...
	if(type !== 'assemble') {
		...
		$('.grid-wrap').on('click', '.goods .ui-icon-ellipsis', function(e){
			...
			$.dialog({
					...
					//content: 'url:http://'+Public.getHostName()+'/settings/goods_batch',
					data: {
						skuMult: skuMult,
						skey:_self.skey,
...
$(function(){
	...
	Public.mod_PageConfig = (function(mod) {
								...
								afterSaveCell: function(rowid,name,val,iRow,iCol) {
									switch(name){
										...
										if(!$.trim(val)) {
											...
											_updatePageConfig($grid[0].id, ['label' ,rowData.name, rowData['label']]);
											return val;
										}
										break;
									}
		...
		function _gridReg(gridId , defColModel, caption){//表格一定要注册默认配置信息
			...
			var g = conf.grids[gridId];
			g.caption = caption;
			g.defColModel = defColModel;//保存默认配置
		...
		function _deleteConfig(key){
			defaultPage.$.post('../basedata/userSetting?action=delete',{
				key : mod.pageId
