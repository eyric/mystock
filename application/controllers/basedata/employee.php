...
class Employee extends CI_Controller {
	...
	public function delete(){
		...
		if (count($data)>0) {
		    $this->mysql_model->get_count(INVOICE,'(salesId='.$id.')')>0 && str_alert(-1,'其中有客户发生业务不可删除');
			$info['isDelete'] = 1;
	...
	public function findByNumberOrName(){
		...
		$offset = $rows*($page-1); 
		$data['data']['totalsize'] = $this->mysql_model->get_count(STAFF,'(1=1) '.$where.'');    //总条数
		$list = $this->mysql_model->get_results(STAFF,'(1=1) '.$where.' order by id desc limit '.$offset.','.$rows.'');  
		foreach ($list as $arr=>$row) {
		    $v[$arr]['id']         = intval($row['id']); 
			$v[$arr]['name']       = $row['name'];
			$v[$arr]['number']     = $row['number'];
