...
class InvTf extends CI_Controller {
	...
	public function tfList(){
		...
		foreach ($list as $arr=>$row) {
		    foreach ($info as $arr1=>$row1) {
			    if ($row1['iid']==$row['id']) {
				    if ($row1['qty']>0) {
						...
						$goods[]           = $row1['invNumber'].' '.$row1['invName'].' '.$row1['invSpec'];
					    $inLocationName[]  = $row1['locationName'];
					} else {
					    $outLocationName[] = $row1['locationName'];
					}
				}
			}
			...
			$v[$arr]['outLocationName']    = $outLocationName;
			$v[$arr]['inLocationName']     = $inLocationName;
		}
		$data['data']['rows']        = $v ;
