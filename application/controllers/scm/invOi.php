...
class InvOi extends CI_Controller {
	...
	//批量审核
    public function batchcheckinvOo() {
	    $this->common_model->checkpurview(103);
	    $id   = str_enhtml($this->input->post('id',TRUE));
		$data = $this->mysql_model->get_results(INVOICE,'(id in('.$id.')) and billType="OO" and checked=0');  
		if (count($data)>0) {
			$info['checked']   = 1;
			$info['checkName'] = $this->jxcsys['name'];
			$sql = $this->mysql_model->update(INVOICE,$info,'(id in('.$id.'))');  
    ...
    public function rsbatchcheckinvOo() {
	    ...
	    $id   = str_enhtml($this->input->post('id',TRUE));
		$data = $this->mysql_model->get_results(INVOICE,'(id in('.$id.')) and billType="OO" and checked=1');  
		if (count($data)>0) {
			$info['checked']   = 0;
			$info['checkName'] = '';
			$sql = $this->mysql_model->update(INVOICE,$info,'(id in('.$id.'))');  
    ...
    //单个反审核
	public function revscheckinvOo() {
	    $this->common_model->checkpurview(104);
	    $data = $this->input->post('postData',TRUE);
		if (strlen($data)>0) {
		    $data = (array)json_decode($data, true); 
	...
	public function generatorPD() {
		...
		if (strlen($info)>0) {
			 ...
			 if (is_array($info['entries'])) {
				...
				$data['data']['items'][0]['entries']     = array_merge(array() , $v);
				...
				$data['data']['items'][1]['entries']     = array_merge(array() , $s);
			}
