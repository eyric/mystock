	...
	public function fundBalance_detailSupplier() {
		...
		foreach ($list1 as $arr=>$row) {
			...
			foreach ($list2 as $arr1=>$row1) {
			    $arr = time() + $arr1;
				if ($row['id']==$row1['buId']) {
				    $sum1 += $a1 = $row1['billType']=='PUR' ? $row1['amount'] : 0;      //采购
					$sum2 += $a2 = $row1['billType']=='PAYMENT' ? $row1['amount'] : 0;  //支付
					$a3 = $row['amount'] + $sum1 - $sum2;
					$v[$arr]['balance']     = $a3;   
	...
	public function fundBalance_detail() {
		...
		foreach ($list1 as $arr=>$row) {
			...
			foreach ($list2 as $arr1=>$row1) {
			    $arr = time() + $arr1;
				if ($row['id']==$row1['buId']) {
				    $sum1 += $a1 = $row1['billType']=='SALE' ? $row1['amount'] : 0; 
					$sum2 += $a2 = $row1['billType']=='RECEIPT' ? $row1['amount'] : 0; 
					$a3 = $row['amount'] + $sum1 - $sum2;
					$v[$arr]['balance']     = $a3;   
					$v[$arr]['billId']      = $row1['id']; 
					$v[$arr]['billNo']      = $row1['billNo']; 
